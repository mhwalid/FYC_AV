...
import {
  RequestTransactionSchemaCreate,
...
} from "../../schema/sharePrice/walletSharePricesSchema.ts";
...
const TransactionController = {
  async getTransactionByUserId(ctx: CustomContext) {
    try {
      ...
      const userTransaction = await transactionService.findByUserId(
        Number(userId),
      );
  ...
  async buySharePrice(ctx: Context) {
    try {
      ...
      const transactionDataRequest: RequestTransactionSchemaCreate = await ctx
        .request.body().value;
      const typeTransaction = "Achat";

      // On vérifie que l'action existe
      const sharePriceResponse = await sharePriceService.findById(
        transactionDataRequest.sharePriceId,
      );
      ...
      const buyValue = sharePriceResponse.data?.value *
        transactionDataRequest.volume;
      ...
      const updateUserWallerResponse = await updateWallet(
      ...
      const newSharePriceVolume = sharePriceResponse.data.volume -
        transactionDataRequest.volume;
      const sharePriceUpdateResponse = await updateSharePrice(
        sharePriceResponse.data,
        newSharePriceVolume,
      );

      if (
        !sharePriceUpdateResponse.success ||
        sharePriceUpdateResponse.sharePriceHistoryId === null
      ) {
      ...
      const userWalletSharePriceResponse = await walletSharePriceService
        .findUserSharePrice(userResponse.data.id, sharePriceResponse.data.id);

      if (
        userWalletSharePriceResponse.success &&
        userWalletSharePriceResponse.data !== null
      ) {
        // Modifie le volume d'action possèdé par l'utilisateur
        if (typeof userWalletSharePriceResponse.data.volume === "string") {
          userWalletSharePriceResponse.data.volume = parseFloat(
            userWalletSharePriceResponse.data.volume,
          );
        }

        const walletSharePriceUpdateResponse = await updateWalletSharePrice(
          userWalletSharePriceResponse.data,
          userWalletSharePriceResponse.data.volume +
            transactionDataRequest.volume,
        );
      ...
      const createdTransaction = await createTransaction(
        ...
        typeTransaction,
        userResponse.data.id,
        sharePriceUpdateResponse.sharePriceHistoryId,
      );
  ...
  async sellSharePrice(ctx: CustomContext) {
    try {
      ...
      if (
        userWalletSharePriceResponse.data.volume < transactionDataRequest.volume
      ) {
        ctx.response.status = 400;
        ctx.response.body = {
          success: false,
      ...
      const sellValue: number = sharePriceResponse.data?.value *
        transactionDataRequest.volume;
      ...
      if (typeof sharePriceResponse.data.volume === "string") {
        sharePriceResponse.data.volume = parseFloat(
          sharePriceResponse.data.volume,
        );
      }
      const newSharePriceVolume: number = sharePriceResponse.data.volume +
        transactionDataRequest.volume;
...
async function updateSharePrice(
  sharePriceData: SharePriceSchema,
  newVolume: number,
  ...
  const sharePriceUpdate: SharePriceSchemaUpdate = {
    ...
    value: calculateNewValue(
      sharePriceData.value,
      sharePriceData.volume,
      newVolume,
    ),
    volume: newVolume,
  };
...
async function updateWalletSharePrice(
  userWalletSharePriceDate: WalletSharePriceSchema,
  volume: number,
): Promise<UpdateByIdResponse<WalletSharePriceSchema>> {
  const walletSharePriceUpdateRequest: WalletSharePriceSchemaUpdate = {
    id: userWalletSharePriceDate.id,
    volume: volume,
  };
  return await walletSharePriceService.updateVolumeById(
    walletSharePriceUpdateRequest,
  );
}
