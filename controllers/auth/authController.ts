...
const AuthController = {
  async register(ctx: Context) {
    try {
      ...
      const registrationResponse = await AuthenticationService.register(
        userData,
      );

      // Ajoute le token dans les cookies utilisateurs
      if (registrationResponse.jwtToken !== undefined) {
        CookiesHandler.setLoginCookies(
          ctx,
          registrationResponse.jwtToken,
          registrationResponse.roleName ?? "",
          registrationResponse.userId as number,
        );
      }
    ...
    try {
      ...
      if (loginResponse.jwtToken !== null) {
        CookiesHandler.setLoginCookies(
          ...
          loginResponse.userId as number,
        );
      }
