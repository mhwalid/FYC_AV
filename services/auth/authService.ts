...
import {
  LoginResponse,
  RegisterResponse,
} from "../../schema/auth/authSchema.ts";
...
const AuthentificationService = {
    ...
    try {
      ...
      if (createUserResponse.success) {
        ...
        if (createUserResponse.info !== null) {
          const roleResponse = await roleService.findByUserId(
            createUserResponse.info?.lastInsertId,
          );
  ...
  async login(userLogin: UserSchemaLogin): Promise<LoginResponse> {
    try {
      const resultExistUserEmail = await userService.findByEmail(
        userLogin.email,
      );
      ...
      if (resultExistUserEmail.data !== null) {
        passwordMatch = await bcrypt.compare(
          userLogin.password,
          resultExistUserEmail.data.password,
        );
      }
      ...
      if (!resultExistUserEmail.success || !passwordMatch) {
        return {
          success: false,
          message:
            "Erreur lors de la connexion : Informations de connexion invalides",
          httpCode: 400,
        };
      }
